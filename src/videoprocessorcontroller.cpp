...
QImage VideoProcessorController::matToQImage(const Mat *image) {
	size_t size = static_cast<size_t>(image->rows * image->cols * 3);
	uchar *data = new uchar[size];

	int count = image->rows * image->cols * 3;
	...
	return QImage(data, image->cols, image->rows, image->step, QImage::Format_RGB888,
			   [](void *data){
